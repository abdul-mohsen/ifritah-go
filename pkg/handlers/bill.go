...
func (h *handler) getBaseBills(page int, pageSize int, q string) []BillBase {
	...
	if q == "" {
		...
		) AS T WHERE user_phone_number like ?  and state >= 0 ORDER BY effective_date DESC LIMIT ? OFFSET ?`
...
func (h *handler) AddBill(c *gin.Context) {
	...
	args := db.CreateBillParams{
		...
		Note:            NewNullStringPointer(request.Note),
		Username:        NewNullStringPointer(request.UserName),
		BuyerID:         sql.NullInt32{},
		UserPhoneNumber: NewNullStringPointer(request.UserPhoneNumber),
	}
