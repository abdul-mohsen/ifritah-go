...
func (h *handler) AddBill(c *gin.Context) {
	...
	if err := qtx.RefreshBillTotals(c.Request.Context(), db.RefreshBillTotalsParams{TargetBillID: uint64(id), TargetID: uint64(id)}); err != nil {
		log.Printf("AddBill: %v", err)
		c.AbortWithError(http.StatusInternalServerError, err)
		return
	}
...
func (h *handler) SubmitDraftBill(c *gin.Context) {
	...
	if err := qtx.RefreshBillTotals(c.Request.Context(), db.RefreshBillTotalsParams{TargetBillID: billID, TargetID: billID}); err != nil {
		log.Printf("SubmitDraftBill: %v", err)
		c.AbortWithError(http.StatusInternalServerError, err)
		return
	}
