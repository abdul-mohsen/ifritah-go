...
SELECT * from(
			...
			WHERE bill.user_phone_number like ?  and state >= 0
			UNION
			SELECT bill.id as id, effective_date, payment_due_date, bill.state as state, discount, sequence_number, user_phone_number, TRUE as bill_type, 0 as credit_state, total, total_vat, total_before_vat from bill_totals as bill WHERE bill.user_phone_number like ?  and state >= 0
