	...
	return func(c *gin.Context) {
		...
		defer func() {
			...
			case status >= http.StatusInternalServerError:
				logger.LogAttrs(c.Request.Context(), slog.LevelError, "http.request.completed", attrs...) // codeql[go/log-injection]
			case status >= http.StatusBadRequest:
				logger.LogAttrs(c.Request.Context(), slog.LevelWarn, "http.request.completed", attrs...) // codeql[go/log-injection]
			default:
				logger.LogAttrs(c.Request.Context(), slog.LevelInfo, "http.request.completed", attrs...) // codeql[go/log-injection]
			}
