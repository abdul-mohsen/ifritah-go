...
SELECT * from(
			SELECT bill.id as id, effective_date, payment_due_date, bill.state as state, sub_total, discount, vat, sequence_number, bill.user_phone_number, TRUE as bill_type, cn.state as credit_state, total, total_vat, total_before_vat
		...
		) AS T WHERE bill.user_phone_number like ?  and state >= 0 ORDER BY id DESC LIMIT ? OFFSET ?;
